...
var onUpdateDatasourceJsonDataOptionChecked = function (props, key, val) { return function (event) {
...
})(exports.DataSourceStatus || (exports.DataSourceStatus = {}));
...
var PanelPlugin = /** @class */ (function (_super) {
    ...
    PanelPlugin.prototype.setCustomFieldConfigs = function (registry) {
...
(function (FieldColorMode) {
    FieldColorMode["Thresholds"] = "thresholds";
    FieldColorMode["Scheme"] = "scheme";
    FieldColorMode["Fixed"] = "fixed";
})(exports.FieldColorMode || (exports.FieldColorMode = {}));
(function (ColorScheme) {
    ColorScheme["BrBG"] = "BrBG";
    ColorScheme["PRGn"] = "PRGn";
    ColorScheme["PiYG"] = "PiYG";
    ColorScheme["PuOr"] = "PuOr";
    ColorScheme["RdBu"] = "RdBu";
    ColorScheme["RdGy"] = "RdGy";
    ColorScheme["RdYlBu"] = "RdYlBu";
    ColorScheme["RdYlGn"] = "RdYlGn";
    ColorScheme["Spectral"] = "Spectral";
    ColorScheme["BuGn"] = "BuGn";
    ColorScheme["BuPu"] = "BuPu";
    ColorScheme["GnBu"] = "GnBu";
    ColorScheme["OrRd"] = "OrRd";
    ColorScheme["PuBuGn"] = "PuBuGn";
    ColorScheme["PuBu"] = "PuBu";
    ColorScheme["PuRd"] = "PuRd";
    ColorScheme["RdPu"] = "RdPu";
    ColorScheme["YlGnBu"] = "YlGnBu";
    ColorScheme["YlGn"] = "YlGn";
    ...
    // interpolateInferno
    // interpolatePlasma
})(exports.ColorScheme || (exports.ColorScheme = {}));
...
(function (AppEvents) {
    AppEvents.alertSuccess = eventFactory('alert-success');
    AppEvents.alertWarning = eventFactory('alert-warning');
    AppEvents.alertError = eventFactory('alert-error');
})(exports.AppEvents || (exports.AppEvents = {}));

(function (PanelEvents) {
    /** Events */
    PanelEvents.refresh = eventFactory('refresh');
    PanelEvents.componentDidMount = eventFactory('component-did-mount');
    PanelEvents.dataError = eventFactory('data-error');
    PanelEvents.dataReceived = eventFactory('data-received');
    PanelEvents.dataSnapshotLoad = eventFactory('data-snapshot-load');
    PanelEvents.editModeInitialized = eventFactory('init-edit-mode');
    PanelEvents.initPanelActions = eventFactory('init-panel-actions');
    PanelEvents.panelChangeView = eventFactory('panel-change-view');
    PanelEvents.panelInitialized = eventFactory('panel-initialized');
    PanelEvents.panelSizeChanged = eventFactory('panel-size-changed');
    PanelEvents.panelTeardown = eventFactory('panel-teardown');
    PanelEvents.render = eventFactory('render');
    PanelEvents.viewModeChanged = eventFactory('view-mode-changed');
})(exports.PanelEvents || (exports.PanelEvents = {}));
...
var toDataFrame = function (data) {
    if (data.hasOwnProperty('fields')) {
        ...
        if (data.hasOwnProperty('rows')) {
            var v = new MutableDataFrame(data);
            var rows = data.rows;
            for (var i = 0; i < rows.length; i++) {
                v.appendRow(rows[i]);
            }
...
var CSVReader = /** @class */ (function () {
    ...
    CSVReader.prototype.readCSV = function (text) {
        ...
        var papacfg = __assign(__assign({}, this.config), { dynamicTyping: false, skipEmptyLines: true, comments: false, step: this.step });
...
function getLogLevelFromKey(key) {
    var level = exports.LogLevel[key.toLowerCase()];
    if (level) {
        return level;
    }
    return exports.LogLevel.unknown;
...
var getColorDefinition = function (hex, theme) {
    return flatten_1(Array.from(getNamedColorPalette().values())).filter(function (definition) { return definition.variants[theme] === hex; })[0];
};
...
var getColorName = function (color, theme) {
    ...
    if (isHex(color)) {
        var definition = getColorDefinition(color, theme || exports.GrafanaThemeType.Dark);
        return definition ? definition.name : undefined;
    }
    return color;
};
var getColorByName = function (colorName) {
    var definition = flatten_1(Array.from(getNamedColorPalette().values())).filter(function (definition) { return definition.name === colorName; });
...
var DataFrameView = /** @class */ (function () {
    ...
    DataFrameView.prototype.toArray = function () {
        var _this = this;
        return new Array(this.data.length).fill(0).map(function (_, i) { return (__assign({}, _this.get(i))); });
    };
    ...
    DataFrameView.prototype.map = function (iterator) {
        var acc = [];
        for (var i = 0; i < this.data.length; i++) {
...
var fieldNamePrefixSep = '🦥: ';
function stripFieldNamePrefix(name) {
    var idx = name.indexOf(fieldNamePrefixSep);
    if (idx > 0) {
...
function grafanaDataFrameToArrowTable(data) {
    ...
    if (table instanceof apacheArrow.Table) {
        return table;
    }
...
function resultsToDataFrames(rsp) {
    ...
    var frames = [];
    try {
        for (var _c = __values(Object.values(rsp.results)), _d = _c.next(); !_d.done; _d = _c.next()) {
            var res = _d.value;
            var r = res;
            if (r.dataframes) {
                try {
                    for (var _e = (e_2 = void 0, __values(r.dataframes)), _f = _e.next(); !_f.done; _f = _e.next()) {
                        var b = _f.value;
...
var numericMacher = {
    ...
    get: function () {
        return fieldTypeMacher.get(exports.FieldType.number);
    },
...
var timeMacher = {
    ...
    get: function () {
        return fieldTypeMacher.get(exports.FieldType.time);
    },
    getOptionsDisplayText: function () {
        return 'Time Fields';
    },
...
function setMarkdownOptions(optionsOverride) {
    marked.setOptions(__assign(__assign({}, defaultMarkedOptions), optionsOverride));
}
...
var fieldMatchers = new Registry(function () {
    return __spread(getFieldPredicateMatchers(), getFieldTypeMatchers(), getFieldNameMatchers());
});
...
var appendTransformer = {
    id: exports.DataTransformerID.append,
    name: 'Append',
    ...
    transformer: function (options) {
        return function (data) {
            ...
            // Add the first row
            var processed = new MutableDataFrame();
            try {
                for (var _b = __values(data[0].fields), _c = _b.next(); !_c.done; _c = _b.next()) {
                    var f = _c.value;
                    processed.addField(__assign(__assign({}, f), { values: __spread(f.values.toArray()) }));
                }
            }
            catch (e_1_1) { e_1 = { error: e_1_1 }; }
            finally {
            ...
            for (var i = 1; i < data.length; i++) {
                ...
                for (var j = 0; j < frame.fields.length; j++) {
                    ...
                    if (!vals) {
                        vals = processed.addField(__assign(__assign({}, src), { values: [] }), startLength).values;
                    }
                    // Add each row
...
function reduceField(options) {
    ...
    if (queue.length === 1 && queue[0].reduce) {
        ...
        field.calcs = __assign(__assign({}, field.calcs), values_1);
        return values_1;
    }
    ...
    field.calcs = __assign(__assign({}, field.calcs), values);
    return values;
}
// ------------------------------------------------------------------------------
//
...
var reduceTransformer = {
    ...
    defaultOptions: {
        reducers: [exports.ReducerID.min, exports.ReducerID.max, exports.ReducerID.mean, exports.ReducerID.last],
    },
    /**
     * Return a modified copy of the series.  If the transform is not or should not
     * be applied, just return the input series
     */
    transformer: function (options) {
        var matcher = options.fields ? getFieldMatcher(options.fields) : alwaysFieldMatcher;
        var calculators = options.reducers && options.reducers.length ? fieldReducers.list(options.reducers) : [];
...
var filterFieldsTransformer = {
    ...
    defaultOptions: {
        include: { id: exports.FieldMatcherID.numeric },
    },
    /**
     * Return a modified copy of the series.  If the transform is not or should not
     * be applied, just return the input series
    ...
    try {
        for (var options_1 = __values(options), options_1_1 = options_1.next(); !options_1_1.done; options_1_1 = options_1.next()) {
            ...
            if (after && after !== processed) {
                try {
                    for (var after_1 = (e_2 = void 0, __values(after)), after_1_1 = after_1.next(); !after_1_1.done; after_1_1 = after_1.next()) {
...
var TransformerRegistry = /** @class */ (function (_super) {
    __extends(TransformerRegistry, _super);
...
(function (dateMath) {
    ...
    dateMath.isMathString = isMathString;
    /**
    ...
    dateMath.parseDateMath = parseDateMath;
})(exports.dateMath || (exports.dateMath = {}));
...
(function (rangeUtil) {
    ...
    rangeUtil.getRelativeTimesList = getRelativeTimesList;
    ...
    rangeUtil.describeTextRange = describeTextRange;
    /**
    ...
    function describeTimeRange(range) {
        ...
        if (isDateTime(range.from)) {
            var toMoment = exports.dateMath.parse(range.to, true);
            return toMoment ? formatDate(range.from) + ' to ' + toMoment.fromNow() : '';
        }
        if (isDateTime(range.to)) {
            var from = exports.dateMath.parse(range.from, false);
    ...
    rangeUtil.describeTimeRange = describeTimeRange;
    rangeUtil.isValidTimeSpan = function (value) {
...
})(exports.rangeUtil || (exports.rangeUtil = {}));
...
var MS_DATE_TIME_FORMAT = 'YYYY-MM-DD HH:mm:ss.SSS';
...
function toDateTimeValueFormatter(pattern, todayPattern) {
    return function (value, decimals, scaledDecimals, timeZone) {
        ...
        var time = isUtc ? toUtc(value) : dateTime(value);
        if (todayPattern) {
...
var dateTimeAsIso = toDateTimeValueFormatter('YYYY-MM-DD HH:mm:ss', 'HH:mm:ss');
var dateTimeAsUS = toDateTimeValueFormatter('MM/DD/YYYY h:mm:ss a', 'h:mm:ss a');
...
function getScaleCalculator(field, theme) {
    ...
    var thresholds = config.thresholds, color = config.color;
    var fixedColor = color && color.mode === exports.FieldColorMode.Fixed && color.fixedColor
        ? getColorFromHexRgbOrName(color.fixedColor, themeType)
        : undefined;
    // Should we calculate the percentage
    var percentThresholds = thresholds && thresholds.mode === exports.ThresholdsMode.Percentage;
    var useColorScheme = color && color.mode === exports.FieldColorMode.Scheme;
    if (percentThresholds || useColorScheme) {
        // Calculate min/max if required
        var min_1 = config.min;
        var max = config.max;
        if (!isNumber_1(min_1) || !isNumber_1(max)) {
            if (field.values && field.values.length) {
                var stats = reduceField({ field: field, reducers: [exports.ReducerID.min, exports.ReducerID.max] });
                if (!isNumber_1(min_1)) {
                    min_1 = stats[exports.ReducerID.min];
                }
                if (!isNumber_1(max)) {
    ...
    if (thresholds) {
        return function (value) {
            var threshold = getActiveThreshold(value, thresholds.steps);
            var color = (fixedColor !== null && fixedColor !== void 0 ? fixedColor : (threshold ? getColorFromHexRgbOrName(threshold.color, themeType) : undefined));
            return {
                threshold: threshold,
                color: color,
            };
...
function getDisplayProcessor(options) {
    var _a;
    if (!options || ___default.isEmpty(options) || !options.field) {
        return toStringProcessor;
    }
    var field = options.field;
    var config = (_a = field.config, (_a !== null && _a !== void 0 ? _a : {}));
    ...
    var formatFunc = getValueFormat(config.unit || 'none');
    var scaleFunc = getScaleCalculator(field, options.theme);
    return function (value) {
        var mappings = config.mappings;
        var text = ___default.toString(value);
        var numeric = toNumber$1(value);
        ...
        if (!isNaN(numeric)) {
            if (shouldFormat && !___default.isBoolean(value)) {
...
function toNumber$1(value) {
    ...
    return ___default.toNumber(value);
}
function toStringProcessor(value) {
    return { text: ___default.toString(value), numeric: toNumber$1(value) };
}
function getDecimalsForValue(value, decimalOverride) {
    if (___default.isNumber(decimalOverride)) {
...
var standardFieldConfigEditorRegistry = new Registry();
            ...
            try {
                for (var _c = (e_2 = void 0, __values(frame.fields)), _d = _c.next(); !_d.done; _d = _c.next()) {
                    ...
                    if (field.type === exports.FieldType.number) {
                        ...
                        if (stats[exports.ReducerID.min] < min) {
                            min = stats[exports.ReducerID.min];
                        }
                        if (stats[exports.ReducerID.max] > max) {
                            max = stats[exports.ReducerID.max];
                        }
                    }
                }
...
function applyFieldOverrides(options) {
    ...
    var source = options.fieldOptions;
    if (!source) {
        return options.data;
    }
    var range = undefined;
    ...
    if (source.overrides) {
        try {
            for (var _b = __values(source.overrides), _c = _b.next(); !_c.done; _c = _b.next()) {
                var rule = _c.value;
    ...
    return options.data.map(function (frame, index) {
        ...
        var scopedVars = {
            __series: { text: 'Series', value: { name: name } },
        };
        var fields = frame.fields.map(function (field, fieldIndex) {
            var e_4, _a, e_5, _b;
            ...
            var config = __assign(__assign({}, field.config), { scopedVars: scopedVars }) || {};
            var context = {
                field: field,
                data: options.data,
            ...
            try {
                ...
                for (var override_1 = __values(override), override_1_1 = override_1.next(); !override_1_1.done; override_1_1 = override_1.next()) {
                    ...
                    if (rule.match(field)) {
                        try {
                            for (var _c = (e_5 = void 0, __values(rule.properties)), _d = _c.next(); !_d.done; _d = _c.next()) {
                                var prop = _d.value;
            ...
            if (options.autoMinMax && field.type === exports.FieldType.number) {
                if (!isNumber_1(config.min) || !isNumber_1(config.max)) {
                    if (!range) {
            ...
            var f = __assign(__assign({}, field), { config: config,
                type: type });
...
function setDynamicConfigValue(config, value, context) {
    var _a, _b;
    var reg = value.custom ? context.custom : standardFieldConfigEditorRegistry;
    var item = (_a = reg) === null || _a === void 0 ? void 0 : _a.getIfExists(value.prop);
    ...
    if (remove) {
        if (value.custom) {
            delete ((_b = config) === null || _b === void 0 ? void 0 : _b.custom)[value.prop];
        }
        else {
            delete config[value.prop];
        }
    }
    else {
        if (value.custom) {
        ...
        try {
            for (var keys_1 = __values(keys), keys_1_1 = keys_1.next(); !keys_1_1.done; keys_1_1 = keys_1.next()) {
                ...
                if (key === 'custom') {
                    ...
                    var customKeys = Object.keys(defaults.custom);
                    try {
                        for (var customKeys_1 = (e_7 = void 0, __values(customKeys)), customKeys_1_1 = customKeys_1.next(); !customKeys_1_1.done; customKeys_1_1 = customKeys_1.next()) {
                            var customKey = customKeys_1_1.value;
                        ...
                        try {
                            if (customKeys_1_1 && !customKeys_1_1.done && (_b = customKeys_1.return)) _b.call(customKeys_1);
                        }
                        finally { if (e_7) throw e_7.error; }
                    }
                }
...
source, key, registry, context) {
    var currentConfig = destination[key];
    if (currentConfig === null || currentConfig === undefined) {
        var item = registry.getIfExists(key);
        if (!item) {
            console.warn("No processor available for " + key + " config  property");
            return;
        }
        if (item && item.shouldApply(context.field)) {
            var val = item.process(source[key], context, item.settings);
...
function validateFieldConfig(config) {
    ...
    if (thresholds) {
        if (!thresholds.mode) {
            thresholds.mode = exports.ThresholdsMode.Absolute;
        }
        if (!thresholds.steps) {
            thresholds.steps = [];
        }
    ...
    if (!config.color) {
        if (thresholds) {
            config.color = {
                mode: exports.FieldColorMode.Thresholds,
            };
        }
        // No Color settings
    }
        ...
        if (color.mode === exports.FieldColorMode.Scheme) {
            if (!color.schemeName) {
                color.schemeName = exports.ColorScheme.BrBG;
            }
        }
        else {
...
var getFieldDisplayValues = function (options) {
    var e_2, _a;
    var _b;
    var replaceVariables = options.replaceVariables, fieldOptions = options.fieldOptions;
    var calcs = fieldOptions.calcs.length ? fieldOptions.calcs : [exports.ReducerID.last];
    var values = [];
    if (options.data) {
        var data = applyFieldOverrides(options);
        var hitLimit = false;
        ...
        for (var s = 0; s < data.length && !hitLimit; s++) {
            ...
            for (var i = 0; i < series.fields.length && !hitLimit; i++) {
                ...
                var display = (_b = field.display, (_b !== null && _b !== void 0 ? _b : getDisplayProcessor({
                    field: field,
                    theme: options.theme,
                })));
                ...
                if (fieldOptions.values) {
                    ...
                    for (var j = 0; j < field.values.length; j++) {
                        ...
                        displayValue.title = replaceVariables(title, __assign(__assign({}, field.config.scopedVars), scopedVars));
                        values.push({
                            name: name,
                            field: config,
                    ...
                    try {
                        for (var calcs_1 = (e_2 = void 0, __values(calcs)), calcs_1_1 = calcs_1.next(); !calcs_1_1.done; calcs_1_1 = calcs_1.next()) {
...
exports.getColorDefinition = getColorDefinition;
...
exports.resultsToDataFrames = resultsToDataFrames;
...
exports.transformersRegistry = transformersRegistry;
...
exports.MS_DATE_TIME_FORMAT = MS_DATE_TIME_FORMAT;
